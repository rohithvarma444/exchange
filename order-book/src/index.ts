...
app.post('/api/v1/orders', async (req: any, res: any) => {
    ...
    const {baseAsset, quoteAsset,price, quantity, side,kind} = req.body;
    ...
    const { executedQty, fills } = fillOrder(orderId, quantity, side, kind);
...
function getOrderId(): string {Math.random
